...
export const activityLogService = {
  create: async (payload: CreateLogPayload): Promise<IActivityLog> => {
    const log = await ActivityLog.create(
      payload as unknown as Partial<IActivityLog>,
    );
    return log;
  },
  ...
  ): Promise<mongoose.PaginateResult<IActivityLog>> => {
    const {
      user: userFilter,
      module,
      startDate,
  ...
  deleteById: async (
    ...
    if (
      ...
      (log.user as Types.ObjectId).toHexString() !== user._id
    ) {
      throw new ApiError(
  ...
  bulkDelete: async (
    ...
    if (!Array.isArray(ids) || ids.length === 0) {
      throw new ApiError(
        ...
        "Provide a non-empty array of IDs",
      );
    }
