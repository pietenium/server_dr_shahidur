...
export const searchService = {
  ...
  universalSearch: async (query: SearchQuery): Promise<UniversalSearchResult> => {
    ...
    const [articles, research, testimonials] = await Promise.all([
      (!type || type === "article") ? searchArticles(searchTerms, limit) : Promise.resolve([] as ArticleSearchResult[]),
      (!type || type === "research") ? searchResearch(searchTerms, limit) : Promise.resolve([] as ResearchSearchResult[]),
      (!type || type === "testimonial") ? searchTestimonials(searchTerms, limit) : Promise.resolve([] as TestimonialSearchResult[]),
    ]);
 ...
 */
async function searchArticles(q: string, limit: number): Promise<ArticleSearchResult[]> {
  try {
 ...
 */
async function searchResearch(q: string, limit: number): Promise<ResearchSearchResult[]> {
  try {
 ...
 */
async function searchTestimonials(q: string, limit: number): Promise<TestimonialSearchResult[]> {
  try {
