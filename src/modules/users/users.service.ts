...
export const usersService = {
  ...
  getAllUsers: async (
    ...
    const result = await (User as unknown as PaginateModel<IUser>).paginate(
      filter,
      {
        page,
        limit,
  ...
  inviteModerator: async (
    payload: InviteModeratorPayload,
  ): Promise<SafeUser> => {
    if (typeof payload.email !== "string") {
      throw new ApiError(StatusCodes.BAD_REQUEST, "Invalid email format");
    }
  ...
  toggleUserActive: async (
    ...
    if (user.role === ROLES.ADMIN) {
      throw new ApiError(
        StatusCodes.FORBIDDEN,
        "Admin account status cannot be toggled",
      );
    }
